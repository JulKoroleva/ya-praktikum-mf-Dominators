...
async function createServer() {
  ...
  if (isDev) {
    ...
    app.use(
      express_1.default.static(path_1.default.join(clientPath, 'dist/client'), { index: false }),
    );
  }
  app.get('*', async (req, res, next) => {
    ...
    try {
      ...
      if (vite) {
        ...
        render = (await vite.ssrLoadModule(path_1.default.join(clientPath, 'src/entry-server.tsx')))
          .render;
      } else {
        template = await promises_1.default.readFile(
        ...
        render = (
          await ((_a = pathToServer), Promise.resolve().then(() => __importStar(require(_a))))
        ).render;
      }
      ...
      const html = template.replace(`<!--ssr-outlet-->`, appHtml).replace(
        `<!--ssr-initial-state-->`,
        `<script>window.APP_INITIAL_STATE = ${(0, serialize_javascript_1.default)(initialState, {
          isJSON: true,
        })}</script>`,
      );
